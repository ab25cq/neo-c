...
struct sNode* string_node_v10(char* buf, char* head, int head_sline, struct sInfo* info  )
{
    ...
    if(charp_operator_equals(buf,"do")) {
        ...
        (come_push_stackframe("17do_while.nc", 71, 0),__exception_result_var_b1=expected_next_character(40,info), come_pop_stackframe(), __exception_result_var_b1);
        expression_node=(struct sNode*)come_increment_ref_count(expression_v13(info,0));
        (come_push_stackframe("17do_while.nc", 76, 1),__exception_result_var_b2=expected_next_character(41,info), come_pop_stackframe(), __exception_result_var_b2);
        _inf_value1=(struct sNode*)come_calloc(1, sizeof(struct sNode), "17do_while.nc", 78, "struct sNode");
        _inf_obj_value1=(struct sDoWhileNode*)come_increment_ref_count(((struct sDoWhileNode*)(__right_value1=sDoWhileNode_initialize((struct sDoWhileNode*)come_increment_ref_count((struct sDoWhileNode*)come_calloc(1, sizeof(struct sDoWhileNode)*(1), "17do_while.nc", 78, "struct sDoWhileNode*")),(struct sNode*)come_increment_ref_count(expression_node),block,info))));
