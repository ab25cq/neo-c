...
static Type specifier(int *sclass) {
    int cls, cons, sign, size, type_, vol;
    Type ty = NULL;

    cls = vol = cons = sign = size = type_ = 0;
    if (sclass == NULL)
        cls = AUTO;
    for (;;) {
        int *p, tt = t;
        ...
        case DOUBLE:   p = &type_; ty = tsym->type;
                                  t = gettok();      break;
        case ENUM:     p = &type_; ty = enumdcl();    break;
        case STRUCT:
        case UNION:    p = &type_; ty = structdcl(t); break;
        case ID:
            if (istypename(t, tsym) && type_ == 0
            && sign == 0 && size == 0) {
                use(tsym, src);
                ty = tsym->type;
                if (isqual(ty)
    ...
    if (size == SHORT     && type_ != INT
    ||  size == LONG+LONG && type_ != INT
    ||  size == LONG      && type_ != INT && type_ != DOUBLE
    ||  sign && type_ != INT && type_ != CHAR)
        error("invalid type specification\n");
    if (type_ == CHAR && sign)
        ty = sign == UNSIGNED ? unsignedchar : signedchar;
    else if (size == SHORT)
        ty = sign == UNSIGNED ? unsignedshort : shorttype;
    else if (size == LONG && type_ == DOUBLE)
        ty = longdouble;
    ...
    else if (sign == UNSIGNED && type_ == INT)
        ty = unsignedtype;
    if (cons == CONST)
        ty = qual(CONST, ty);
    if (vol  == VOLATILE)
...
static Type tnode(int op, Type type_) {
    Type ty;
    ...
    if (kind[t] == STATIC || istypename(t, tsym)) {
        ...
        for (;;) {
            ...
            if (id == NULL)
                id = stringd(n);
            if (ty != voidtype)
                list_ = append(dclparam(sclass, id, ty, &src), list_);
        ...
        fty->u.f.proto = newarray(length(list_) + 1,
            sizeof (Type *), PERM);
        params = ltov(&list_, FUNC);
        for (n = 0; params[n]; n++)
