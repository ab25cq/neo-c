...
_conditional_value_X0;})) {
    ...
    if(field_type==((void*)0)) {
        ...
        come_value->type=(struct sType*)come_increment_ref_count((come_push_stackframe("23field.nc", 423, 2),__exception_result_var_b3=(come_push_stackframe("23field.nc", 423, 0),__exception_result_var_b1=sType_initialize((struct sType*)come_increment_ref_count((struct sType*)come_calloc(1, sizeof(struct sType)*(1), "23field.nc", 423, "struct sType*")),(char*)come_increment_ref_count(xsprintf("void")),0,info,0), come_pop_stackframe(), __exception_result_var_b1), come_pop_stackframe(), __exception_result_var_b3));
    ...
    while(1) {
        ...
        if(!node->terminated(node->_protocol_obj)&&range_array&&(*info->p==92&&*(info->p+1)==91||*info->p==91)) {
            ...
            if(*info->p==46&&*(info->p+1)==46) {
                ...
                (come_push_stackframe("23field.nc", 947, 3),__exception_result_var_b4=expected_next_character(93,info), come_pop_stackframe(), __exception_result_var_b4);
