...
then
    make -C code2
    (echo code && cd code && make test && if [ "${RUN_EXTERNAL_CORPUS:-0}" = "1" ]; then make external-corpus; fi) && (echo vin && cd vin && sh fast_build.sh) && (echo zed && cd zed && make && sudo make install) && (echo shsh && cd shsh && make && sudo make install) && (echo mf && cd mf && make && sudo make install)
else
    make -C code2
    make -C code3
    (echo code && cd code && make test && if [ "${RUN_EXTERNAL_CORPUS:-0}" = "1" ]; then make external-corpus; fi) && (echo vin && cd vin && sh fast_build.sh) && (echo zed && cd zed && make && sudo make install) && (echo shsh && cd shsh && make && sudo make install) && (echo mf && cd mf && make && sudo make install)
