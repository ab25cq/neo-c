...
class sAutomaticallyUnwrapNode extends sNodeBase
{
    ...
    bool compile(sInfo* info)
    {
        ...
        if(info.come_fun && ((strlen(info.come_fun.mName) > strlen("ref$") && memcmp(info.come_fun.mName, "ref$", strlen("ref$")) == 0) || (strlen(info.come_fun.mName) > strlen("optional$") && memcmp(info.come_fun.mName, "optional$", strlen("optional$")) == 0) || (strlen(info.come_fun.mName) > strlen("span$") && memcmp(info.come_fun.mName, "span$", strlen("span$")) == 0)))
        {
            ...
            if(come_value.type.mNoSolvedGenericsType.mClass.mName === "ref" || come_value.type.mNoSolvedGenericsType.mClass.mName === "optional" || come_value.type.mNoSolvedGenericsType.mClass.mName === "span")
            {
                sNode*% obj = node;
                list<tuple2<string, sNode*%>*%>*% params =  new list<tuple2<string, sNode*%>*%>();
