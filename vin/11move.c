...
static _Bool line_is_function_top(char* line);
static _Bool line_is_unindented_function_top(char* line);
static _Bool line_is_blank(char* line);
static _Bool line_starts_with_keyword(char* line, char* keyword);
static _Bool line_is_function_signature(char* line);
static _Bool line_is_method_top(char* line);
static _Bool line_is_function_bottom(char* line);
...
void ViWin_gotoMethodTop(struct ViWin*  self  , struct Vi*  nvi  );
...
static _Bool line_is_function_top(char* line)
{
    struct neo_frame fr; fr.prev = neo_current_frame; fr.fun_name = "line_is_function_top"; fr.frame_id = ++neo_frame_id; neo_current_frame = &fr;
    int head;
    ...
    head=0;
    while(line[head]==32||line[head]==9) {
        head++;
    }
        neo_current_frame = fr.prev;
    return line[head]==123;
    neo_current_frame = fr.prev;
}

static _Bool line_is_unindented_function_top(char* line)
{
    struct neo_frame fr; fr.prev = neo_current_frame; fr.fun_name = "line_is_unindented_function_top"; fr.frame_id = ++neo_frame_id; neo_current_frame = &fr;
        neo_current_frame = fr.prev;
    return line!=((void*)0)&&line[0]==123;
    neo_current_frame = fr.prev;
}

static _Bool line_is_blank(char* line)
{
    struct neo_frame fr; fr.prev = neo_current_frame; fr.fun_name = "line_is_blank"; fr.frame_id = ++neo_frame_id; neo_current_frame = &fr;
    int i;
    memset(&i, 0, sizeof(i));
    if(line==((void*)0)) {
                neo_current_frame = fr.prev;
...
static _Bool line_starts_with_keyword(char* line, char* keyword)
{
    struct neo_frame fr; fr.prev = neo_current_frame; fr.fun_name = "line_starts_with_keyword"; fr.frame_id = ++neo_frame_id; neo_current_frame = &fr;
    int len;
    char c;
    ...
    c=line[len];
        neo_current_frame = fr.prev;
    return c==0||c==32||c==9||c==40;
    neo_current_frame = fr.prev;
}

static _Bool line_is_function_signature(char* line)
{
    struct neo_frame fr; fr.prev = neo_current_frame; fr.fun_name = "line_is_function_signature"; fr.frame_id = ++neo_frame_id; neo_current_frame = &fr;
    int head;
    char* lparen;
    char* rparen;
    int name_pos;
    int name_head;
    memset(&head, 0, sizeof(head));
    memset(&lparen, 0, sizeof(lparen));
    memset(&rparen, 0, sizeof(rparen));
    memset(&name_pos, 0, sizeof(name_pos));
    memset(&name_head, 0, sizeof(name_head));
    if(line==((void*)0)) {
                neo_current_frame = fr.prev;
        return (_Bool)0;
    }
    ...
    if(line_starts_with_keyword(line+head,"if")||line_starts_with_keyword(line+head,"for")||line_starts_with_keyword(line+head,"while")||line_starts_with_keyword(line+head,"switch")||line_starts_with_keyword(line+head,"else")||line_starts_with_keyword(line+head,"do")) {
                neo_current_frame = fr.prev;
        return (_Bool)0;
    }
    lparen=strstr(line+head,"(");
    rparen=strstr(line+head,")");
    if(lparen==((void*)0)||rparen==((void*)0)||lparen>=rparen) {
                neo_current_frame = fr.prev;
        return (_Bool)0;
    }
    name_pos=lparen-line-1;
    while(name_pos>=head&&(line[name_pos]==32||line[name_pos]==9||line[name_pos]==42||line[name_pos]==38)) {
        name_pos--;
    }
    if(name_pos<head) {
                neo_current_frame = fr.prev;
    ...
    if(!(xisalpha(line[name_pos])||xisdigit(line[name_pos])||line[name_pos]==95||line[name_pos]==58)) {
                neo_current_frame = fr.prev;
        return (_Bool)0;
    }
    name_head=name_pos;
    while(name_head>=head&&(xisalpha(line[name_head])||xisdigit(line[name_head])||line[name_head]==95||line[name_head]==58)) {
        name_head--;
    }
    while(name_head>=head&&(line[name_head]==32||line[name_head]==9||line[name_head]==42||line[name_head]==38)) {
        name_head--;
    }
        neo_current_frame = fr.prev;
...
static _Bool line_is_method_top(char* line)
{
    struct neo_frame fr; fr.prev = neo_current_frame; fr.fun_name = "line_is_method_top"; fr.frame_id = ++neo_frame_id; neo_current_frame = &fr;
    int tail;
    ...
    while(tail>=0&&xisspace(line[tail])) {
        ...
        neo_current_frame = fr.prev;
    return tail>=0&&line[tail]==123;
    neo_current_frame = fr.prev;
}

static _Bool line_is_function_bottom(char* line)
{
    struct neo_frame fr; fr.prev = neo_current_frame; fr.fun_name = "line_is_function_bottom"; fr.frame_id = ++neo_frame_id; neo_current_frame = &fr;
        neo_current_frame = fr.prev;
    return line!=((void*)0)&&line[0]==125;
    neo_current_frame = fr.prev;
}

void ViWin_toggleBraceForward(struct ViWin*  self  , int  head  , int  tail  )
{
    struct neo_frame fr; fr.prev = neo_current_frame; fr.fun_name = "ViWin_toggleBraceForward"; fr.frame_id = ++neo_frame_id; neo_current_frame = &fr;
    int cursor_y;
    int cursor_x;
    int nest;
    ...
    cursor_y=self->scroll+self->cursorY;
    cursor_x=-1;
    nest=0;
    line=(int* )come_increment_ref_count(list$1int$ph_item(self->texts,cursor_y,((void*)0)), "11move.nc", 134, 7);
    p=line+self->cursorX+1;
    while(p<line+wstring_length(line)) {
        if(*p==tail) {
            if(nest==0) {
...
({(_conditional_value_X0=(_o2_saved_1=(struct list$1int$ph*)come_increment_ref_count(list$1int$ph_sublist(self->texts,self->scroll+self->cursorY+1,-1), "11move.nc", 165, 42),it=list$1int$ph_begin(_o2_saved_1)));_conditional_value_X0;})        ;({(_conditional_value_X1=(!list$1int$ph_end(_o2_saved_1)));_conditional_value_X1;});({(_conditional_value_X2=(it=list$1int$ph_next(_o2_saved_1)));_conditional_value_X2;})){
            p_4=it;
            while(p_4<it+wstring_length(it)) {
                if(*p_4==tail) {
                    if(nest==0) {
                        ViWin_saveReturnPoint(self);
                        ...
                        ViWin_centeringCursor(self);
                                                come_call_finalizer(list$1int$ph$p_finalize, _o2_saved_1, (void*)0, (void*)0, 0, 0, 0, (void*)0, "11move.nc}", 177, 43);
                        (line = come_decrement_ref_count(line, (void*)0, (void*)0, 0, 0, (void*)0, "11move.nc", 177, 44));
                        neo_current_frame = fr.prev;
                        return;
                    }
                    p_4++;
        ...
        come_call_finalizer(list$1int$ph$p_finalize, _o2_saved_1, (void*)0, (void*)0, 0, 0, 0, (void*)0, "11move.nc}", 197, 45);
    }
    (line = come_decrement_ref_count(line, (void*)0, (void*)0, 0, 0, (void*)0, "11move.nc", 199, 46));
    neo_current_frame = fr.prev;
}
...
static int*  list$1int$ph_item(struct list$1int$ph* self, int position, int*  default_value  )
{
    ...
    memset(&i, 0, sizeof(i));
    if(self==((void*)0)) {
                __result_obj__0 = (int* )come_increment_ref_count(default_value, "/usr/local/include/neo-c.h", 1806, 1);
        neo_current_frame = fr.prev;
        (__result_obj__0 = come_decrement_ref_count(__result_obj__0, (void*)0, (void*)0, 0, 1, (void*)0, "/usr/local/include/neo-c.h", 1806, 2));
    ...
    while(it!=((void*)0)) {
        if(position==i) {
                        __result_obj__0 = (int* )come_increment_ref_count(it->item, "/usr/local/include/neo-c.h", 1817, 3);
            neo_current_frame = fr.prev;
            (__result_obj__0 = come_decrement_ref_count(__result_obj__0, (void*)0, (void*)0, 0, 1, (void*)0, "/usr/local/include/neo-c.h", 1817, 4));
            return __result_obj__0;
        }
        it=it->next;
        i++;
    }
        __result_obj__0 = (int* )come_increment_ref_count(default_value, "/usr/local/include/neo-c.h", 1823, 5);
    neo_current_frame = fr.prev;
    (__result_obj__0 = come_decrement_ref_count(__result_obj__0, (void*)0, (void*)0, 0, 1, (void*)0, "/usr/local/include/neo-c.h", 1823, 6));
    return __result_obj__0;
}
...
static struct list$1int$ph* list$1int$ph_sublist(struct list$1int$ph* self, int begin, int tail)
{
    ...
    memset(&i, 0, sizeof(i));
    if(self==((void*)0)) {
                __result_obj__0 = (struct list$1int$ph*)come_increment_ref_count(self, "/usr/local/include/neo-c.h", 2171, 8);
        neo_current_frame = fr.prev;
        come_call_finalizer(list$1int$ph$p_finalize, __result_obj__0, (void*)0, (void*)0, 0, 0, 1, (void*)0, "/usr/local/include/neo-c.h}", 188, 11);
        return __result_obj__0;
    }
    result=(struct list$1int$ph*)come_increment_ref_count(list$1int$ph_initialize((struct list$1int$ph*)come_increment_ref_count((struct list$1int$ph*)come_calloc(1, sizeof(struct list$1int$ph)*(1), "/usr/local/include/neo-c.h", 2174, 12, "struct list$1int$ph*"), "/usr/local/include/neo-c.h", 2174, 16)), "/usr/local/include/neo-c.h", 2174, 17);
    if(begin<0) {
        begin+=self->len;
    }
    if(tail<0) {
    ...
    if(begin>=self->len) {
        ...
        __result_obj__0 = (struct list$1int$ph*)come_increment_ref_count(((struct list$1int$ph*)(__right_value1=list$1int$ph_initialize((struct list$1int$ph*)come_increment_ref_count((struct list$1int$ph*)come_calloc(1, sizeof(struct list$1int$ph)*(1), "/usr/local/include/neo-c.h", 2189, 18, "struct list$1int$ph*"), "/usr/local/include/neo-c.h", 2189, 19)))), "/usr/local/include/neo-c.h", 2189, 20);
        come_call_finalizer(list$1int$ph$p_finalize, result, (void*)0, (void*)0, 0, 0, 0, (void*)0, "/usr/local/include/neo-c.h}", 2189, 21);
        come_call_finalizer(list$1int$ph$p_finalize, __right_value1, (void*)0, (void*)0, 0, 1, 0, (void*)0, "/usr/local/include/neo-c.h}", 2189, 22);
        neo_current_frame = fr.prev;
        come_call_finalizer(list$1int$ph$p_finalize, __result_obj__0, (void*)0, (void*)0, 0, 0, 1, (void*)0, "/usr/local/include/neo-c.h}", 2189, 23);
        return __result_obj__0;
    }
    if(tail>=self->len) {
        tail=self->len;
    ...
    while(it!=((void*)0)) {
        if(i>=begin&&i<tail) {
            list$1int$ph_push_back(result,(int* )come_increment_ref_count(it->item, "/usr/local/include/neo-c.h", 2200, 38));
        }
        it=it->next;
        i++;
    }
        __result_obj__0 = (struct list$1int$ph*)come_increment_ref_count(result, "/usr/local/include/neo-c.h", 2206, 39);
    come_call_finalizer(list$1int$ph$p_finalize, result, (void*)0, (void*)0, 0, 0, 1, (void*)0, "/usr/local/include/neo-c.h}", 2206, 40);
    neo_current_frame = fr.prev;
    come_call_finalizer(list$1int$ph$p_finalize, __result_obj__0, (void*)0, (void*)0, 0, 0, 1, (void*)0, "/usr/local/include/neo-c.h}", 2206, 41);
    return __result_obj__0;
}
...
static void list$1int$ph$p_finalize(struct list$1int$ph* self)
{
    ...
    while(it!=((void*)0)) {
        prev_it=it;
        it=it->next;
        come_call_finalizer(list_item$1int$ph$p_finalize, prev_it, (void*)0, (void*)0, 0, 0, 0, (void*)0, "/usr/local/include/neo-c.h}", 1565, 10);
    }
            neo_current_frame = fr.prev;
}
...
static void list_item$1int$ph$p_finalize(struct list_item$1int$ph* self)
{
    ...
    if(self!=((void*)0)&&self->item!=((void*)0)) {
        (self->item = come_decrement_ref_count(self->item, (void*)0, (void*)0, 0, 0, (void*)0, "list_item$1int$ph$p_finalize", 2, 9));
    }
            neo_current_frame = fr.prev;
}
...
static struct list$1int$ph* list$1int$ph_initialize(struct list$1int$ph* self)
{
    ...
    self->tail=((void*)0);
    self->len=0;
        __result_obj__0 = (struct list$1int$ph*)come_increment_ref_count(self, "/usr/local/include/neo-c.h", 1544, 13);
    come_call_finalizer(list$1int$ph$p_finalize, self, (void*)0, (void*)0, 0, 0, 1, (void*)0, "/usr/local/include/neo-c.h}", 1544, 14);
    neo_current_frame = fr.prev;
    come_call_finalizer(list$1int$ph$p_finalize, __result_obj__0, (void*)0, (void*)0, 0, 0, 1, (void*)0, "/usr/local/include/neo-c.h}", 1544, 15);
    return __result_obj__0;
}
...
static struct list$1int$ph* list$1int$ph_push_back(struct list$1int$ph* self, int*  item  )
{
    ...
    if(self==((void*)0)) {
        ...
        (item = come_decrement_ref_count(item, (void*)0, (void*)0, 0, 0, (void*)0, "/usr/local/include/neo-c.h", 1674, 24));
        neo_current_frame = fr.prev;
        return __result_obj__0;
    }
    if(self->len==0) {
        litem=(struct list_item$1int$ph*)come_increment_ref_count(((struct list_item$1int$ph*)(__right_value0=(struct list_item$1int$ph*)come_calloc(1, sizeof(struct list_item$1int$ph)*(1), "/usr/local/include/neo-c.h", 1678, 25, "struct list_item$1int$ph*"))), "/usr/local/include/neo-c.h", 1678, 26);
        litem->prev=((void*)0);
        litem->next=((void*)0);
        __dec_obj1=litem->item,
        litem->item=(int* )come_increment_ref_count(item, "/usr/local/include/neo-c.h", 1682, 28);
        __dec_obj1 = come_decrement_ref_count(__dec_obj1, (void*)0, (void*)0, 0,0, (void*)0, "/usr/local/include/neo-c.h", 1682, 27);
        self->tail=litem;
        self->head=litem;
    }
    else if(self->len==1) {
        __right_value0 = (void*)0;
        litem_0=(struct list_item$1int$ph*)come_increment_ref_count(((struct list_item$1int$ph*)(__right_value0=(struct list_item$1int$ph*)come_calloc(1, sizeof(struct list_item$1int$ph)*(1), "/usr/local/include/neo-c.h", 1688, 29, "struct list_item$1int$ph*"))), "/usr/local/include/neo-c.h", 1688, 30);
        litem_0->prev=self->head;
        litem_0->next=((void*)0);
        __dec_obj2=litem_0->item,
        litem_0->item=(int* )come_increment_ref_count(item, "/usr/local/include/neo-c.h", 1692, 32);
        __dec_obj2 = come_decrement_ref_count(__dec_obj2, (void*)0, (void*)0, 0,0, (void*)0, "/usr/local/include/neo-c.h", 1692, 31);
        self->tail=litem_0;
        self->head->next=litem_0;
    }
    else {
        __right_value0 = (void*)0;
        litem_1=(struct list_item$1int$ph*)come_increment_ref_count(((struct list_item$1int$ph*)(__right_value0=(struct list_item$1int$ph*)come_calloc(1, sizeof(struct list_item$1int$ph)*(1), "/usr/local/include/neo-c.h", 1698, 33, "struct list_item$1int$ph*"))), "/usr/local/include/neo-c.h", 1698, 34);
        litem_1->prev=self->tail;
        litem_1->next=((void*)0);
        __dec_obj3=litem_1->item,
        litem_1->item=(int* )come_increment_ref_count(item, "/usr/local/include/neo-c.h", 1702, 36);
        __dec_obj3 = come_decrement_ref_count(__dec_obj3, (void*)0, (void*)0, 0,0, (void*)0, "/usr/local/include/neo-c.h", 1702, 35);
        self->tail->next=litem_1;
        self->tail=litem_1;
    }
    self->len++;
        __result_obj__0 = self;
    (item = come_decrement_ref_count(item, (void*)0, (void*)0, 0, 0, (void*)0, "/usr/local/include/neo-c.h", 1710, 37));
    neo_current_frame = fr.prev;
    return __result_obj__0;
}
...
void ViWin_toggleBraceBack(struct ViWin*  self  , int  head  , int  tail  )
{
    struct neo_frame fr; fr.prev = neo_current_frame; fr.fun_name = "ViWin_toggleBraceBack"; fr.frame_id = ++neo_frame_id; neo_current_frame = &fr;
    int cursor_y;
    int cursor_x;
    int nest;
    void* __right_value0 = (void*)0;
    ...
    cursor_x=-1;
    nest=0;
    line=(int* )come_increment_ref_count(list$1int$ph_item(self->texts,cursor_y,((void*)0)), "11move.nc", 206, 47);
    p=line+self->cursorX-1;
    while(p>=line) {
        if(*p==head) {
            if(nest==0) {
    ...
    if(cursor_x==-1) {
        ...
        it2=0;
        for(__right_value0 = (void*)0,
({(_conditional_value_X0=(_o2_saved_2=(struct list$1int$ph*)come_increment_ref_count(list$1int$ph_reverse(((struct list$1int$ph*)(__right_value0=list$1int$ph_sublist(self->texts,0,self->scroll+self->cursorY)))), "11move.nc", 236, 59),it=list$1int$ph_begin(_o2_saved_2)));        come_call_finalizer(list$1int$ph$p_finalize, __right_value0, (void*)0, (void*)0, 0, 1, 0, (void*)0, "11move.nc}", 236, 60);
_conditional_value_X0;})        ;({(_conditional_value_X1=(!list$1int$ph_end(_o2_saved_2)));_conditional_value_X1;});({(_conditional_value_X2=(it=list$1int$ph_next(_o2_saved_2)));_conditional_value_X2;})){
            ...
            while(p_5>=it) {
                if(*p_5==head) {
                    if(nest==0) {
                        ...
                        self->cursorX=cursor_x;
                        ViWin_centeringCursor(self);
                                                come_call_finalizer(list$1int$ph$p_finalize, _o2_saved_2, (void*)0, (void*)0, 0, 0, 0, (void*)0, "11move.nc}", 249, 61);
                        (line = come_decrement_ref_count(line, (void*)0, (void*)0, 0, 0, (void*)0, "11move.nc", 249, 62));
                        neo_current_frame = fr.prev;
                        return;
                    }
                    p_5--;
        ...
        come_call_finalizer(list$1int$ph$p_finalize, _o2_saved_2, (void*)0, (void*)0, 0, 0, 0, (void*)0, "11move.nc}", 269, 63);
    }
    (line = come_decrement_ref_count(line, (void*)0, (void*)0, 0, 0, (void*)0, "11move.nc", 271, 64));
    neo_current_frame = fr.prev;
}
...
static struct list$1int$ph* list$1int$ph_reverse(struct list$1int$ph* self)
{
    ...
    memset(&result, 0, sizeof(result));
    memset(&it, 0, sizeof(it));
    result=(struct list$1int$ph*)come_increment_ref_count(list$1int$ph_initialize((struct list$1int$ph*)come_increment_ref_count((struct list$1int$ph*)come_calloc(1, sizeof(struct list$1int$ph)*(1), "/usr/local/include/neo-c.h", 1555, 48, "struct list$1int$ph*"), "/usr/local/include/neo-c.h", 1555, 49)), "/usr/local/include/neo-c.h", 1555, 50);
    if(self==((void*)0)) {
                __result_obj__0 = (struct list$1int$ph*)come_increment_ref_count(result, "/usr/local/include/neo-c.h", 1558, 51);
        come_call_finalizer(list$1int$ph$p_finalize, result, (void*)0, (void*)0, 0, 0, 1, (void*)0, "/usr/local/include/neo-c.h}", 1558, 52);
        neo_current_frame = fr.prev;
        come_call_finalizer(list$1int$ph$p_finalize, __result_obj__0, (void*)0, (void*)0, 0, 0, 1, (void*)0, "/usr/local/include/neo-c.h}", 1558, 53);
        return __result_obj__0;
    }
    it=self->tail;
    while(it!=((void*)0)) {
        __right_value0 = (void*)0;
        list$1int$ph_push_back(result,(int* )come_increment_ref_count((int* )come_memdup(it->item, "/usr/local/include/neo-c.h", 1563, 54, "int* "), "/usr/local/include/neo-c.h", 1563, 55));
        it=it->prev;
    }
        __result_obj__0 = (struct list$1int$ph*)come_increment_ref_count(result, "/usr/local/include/neo-c.h", 1567, 56);
    come_call_finalizer(list$1int$ph$p_finalize, result, (void*)0, (void*)0, 0, 0, 1, (void*)0, "/usr/local/include/neo-c.h}", 1567, 57);
    neo_current_frame = fr.prev;
    come_call_finalizer(list$1int$ph$p_finalize, __result_obj__0, (void*)0, (void*)0, 0, 0, 1, (void*)0, "/usr/local/include/neo-c.h}", 1567, 58);
    return __result_obj__0;
}

void ViWin_toggleCommentForward(struct ViWin*  self  )
{
    struct neo_frame fr; fr.prev = neo_current_frame; fr.fun_name = "ViWin_toggleCommentForward"; fr.frame_id = ++neo_frame_id; neo_current_frame = &fr;
    int head;
    int tail;
    int nest;
    int it2;
    ...
    head=self->scroll+self->cursorY;
    tail=-1;
    nest=0;
    it2=0;
    for(({(_conditional_value_X0=(_o2_saved_3=(struct list$1int$ph*)come_increment_ref_count(list$1int$ph_sublist(self->texts,head,tail), "11move.nc", 279, 65),it=list$1int$ph_begin(_o2_saved_3)));_conditional_value_X0;})    ;({(_conditional_value_X1=(!list$1int$ph_end(_o2_saved_3)));_conditional_value_X1;});({(_conditional_value_X2=(it=list$1int$ph_next(_o2_saved_3)));_conditional_value_X2;})){
        __right_value0 = (void*)0;
        index=string_index(((char* )(__right_value0=wstring_to_string(it))),"*/",-1);
        (__right_value0 = come_decrement_ref_count(__right_value0, (void*)0, (void*)0, 1, 0, (void*)0, "11move.nc", 280, 66));
        __right_value0 = (void*)0;
        index2=string_index(((char* )(__right_value0=wstring_to_string(it))),"/*",-1);
        (__right_value0 = come_decrement_ref_count(__right_value0, (void*)0, (void*)0, 1, 0, (void*)0, "11move.nc", 282, 67));
        if(index2!=-1) {
            nest++;
        }
        if(index!=-1) {
    ...
    come_call_finalizer(list$1int$ph$p_finalize, _o2_saved_3, (void*)0, (void*)0, 0, 0, 0, (void*)0, "11move.nc}", 306, 68);
    neo_current_frame = fr.prev;
}

void ViWin_toggleCommentBackward(struct ViWin*  self  )
{
    struct neo_frame fr; fr.prev = neo_current_frame; fr.fun_name = "ViWin_toggleCommentBackward"; fr.frame_id = ++neo_frame_id; neo_current_frame = &fr;
    int head;
    int tail;
    int nest;
    int it2;
    ...
    memset(&index2, 0, sizeof(index2));
    head=0;
    tail=self->scroll+self->cursorY+1;
    nest=0;
    it2=0;
    for(({(_conditional_value_X0=(_o2_saved_4=(struct list$1int$ph*)come_increment_ref_count(list$1int$ph_reverse(((struct list$1int$ph*)(__right_value0=list$1int$ph_sublist(self->texts,head,tail)))), "11move.nc", 314, 69),it=list$1int$ph_begin(_o2_saved_4)));    come_call_finalizer(list$1int$ph$p_finalize, __right_value0, (void*)0, (void*)0, 0, 1, 0, (void*)0, "11move.nc}", 314, 70);
_conditional_value_X0;})    ;({(_conditional_value_X1=(!list$1int$ph_end(_o2_saved_4)));_conditional_value_X1;});({(_conditional_value_X2=(it=list$1int$ph_next(_o2_saved_4)));_conditional_value_X2;})){
        __right_value0 = (void*)0;
        index=string_index(((char* )(__right_value0=wstring_to_string(it))),"/*",-1);
        (__right_value0 = come_decrement_ref_count(__right_value0, (void*)0, (void*)0, 1, 0, (void*)0, "11move.nc", 315, 71));
        __right_value0 = (void*)0;
        index2=string_index(((char* )(__right_value0=wstring_to_string(it))),"*/",-1);
        (__right_value0 = come_decrement_ref_count(__right_value0, (void*)0, (void*)0, 1, 0, (void*)0, "11move.nc", 317, 72));
        if(index2!=-1) {
            nest++;
        }
        if(index!=-1) {
    ...
    come_call_finalizer(list$1int$ph$p_finalize, _o2_saved_4, (void*)0, (void*)0, 0, 0, 0, (void*)0, "11move.nc}", 341, 73);
    neo_current_frame = fr.prev;
}

void ViWin_gotoBraceEnd_v11(struct ViWin*  self  , struct Vi*  nvi  )
{
    struct neo_frame fr; fr.prev = neo_current_frame; fr.fun_name = "ViWin_gotoBraceEnd_v11"; fr.frame_id = ++neo_frame_id; neo_current_frame = &fr;
    void* __right_value0 = (void*)0;
    int*  line  ;
    int  c  ;
    int  c1  ;
    ...
    line=(int* )come_increment_ref_count(list$1int$ph_item(self->texts,self->scroll+self->cursorY,((void*)0)), "11move.nc", 343, 74);
    c=line[self->cursorX];
    c1=0;
    c2=0;
    if(self->cursorX-1>=0) {
    ...
    if(self->cursorX+1<wstring_length(line)) {
        c2=line[self->cursorX+1];
    }
    switch (    c) {
        case 42:
        if(c1==47) {
            ViWin_toggleCommentForward(self);
        }
        if(c2==47) {
            ViWin_toggleCommentBackward(self);
        }
        break;
        case 47:
        if(c1==42) {
        ...
        break;
        case 40:
        ViWin_toggleBraceForward(self,40,41);
        break;
        case 123:
        ViWin_toggleBraceForward(self,123,125);
        break;
        case 91:
        ViWin_toggleBraceForward(self,91,93);
        break;
        case 60:
        ViWin_toggleBraceForward(self,60,62);
        break;
        case 41:
        ViWin_toggleBraceBack(self,40,41);
        break;
        case 125:
        ViWin_toggleBraceBack(self,123,125);
        break;
        case 93:
        ViWin_toggleBraceBack(self,91,93);
        case 62:
        ViWin_toggleBraceBack(self,60,62);
        break;
    }
    (line = come_decrement_ref_count(line, (void*)0, (void*)0, 0, 0, (void*)0, "11move.nc", 407, 75));
    neo_current_frame = fr.prev;
}

void ViWin_gotoFunctionTop(struct ViWin*  self  , struct Vi*  nvi  )
{
    struct neo_frame fr; fr.prev = neo_current_frame; fr.fun_name = "ViWin_gotoFunctionTop"; fr.frame_id = ++neo_frame_id; neo_current_frame = &fr;
    int it2;
    void* __right_value0 = (void*)0;
    void* __right_value1 = (void*)0;
    ...
    memset(&i, 0, sizeof(i));
    memset(&it2_6, 0, sizeof(it2_6));
    memset(&next_line, 0, sizeof(next_line));
    it2=0;
    for(({(_conditional_value_X0=(_o2_saved_5=(struct list$1int$ph*)come_increment_ref_count(list$1int$ph_reverse(((struct list$1int$ph*)(__right_value0=list$1int$ph_sublist(self->texts,0,self->scroll+self->cursorY)))), "11move.nc", 410, 76),it=list$1int$ph_begin(_o2_saved_5)));    come_call_finalizer(list$1int$ph$p_finalize, __right_value0, (void*)0, (void*)0, 0, 1, 0, (void*)0, "11move.nc}", 410, 77);
_conditional_value_X0;})    ;({(_conditional_value_X1=(!list$1int$ph_end(_o2_saved_5)));_conditional_value_X1;});({(_conditional_value_X2=(it=list$1int$ph_next(_o2_saved_5)));_conditional_value_X2;})){
        y=self->cursorY+self->scroll-it2-1;
        __right_value0 = (void*)0;
        prev_line=(int* )come_increment_ref_count(__builtin_wstring("","11move.nc",412), "11move.nc", 412, 78);
        for(i=y-1        ;i>=0;i--){
            __right_value0 = (void*)0;
            __right_value1 = (void*)0;
            it2_6=(int* )come_increment_ref_count(list$1int$ph_item(self->texts,i,((int* )(__right_value0=__builtin_wstring("","11move.nc",415)))), "11move.nc", 415, 79);
            (__right_value0 = come_decrement_ref_count(__right_value0, (void*)0, (void*)0, 1, 0, (void*)0, "11move.nc", 415, 80));
            if(__right_value0 = (void*)0,
({(_conditional_value_X3=(!line_is_blank(((char* )(__right_value0=wstring_to_string(it2_6))))));            (__right_value0 = come_decrement_ref_count(__right_value0, (void*)0, (void*)0, 1, 0, (void*)0, "11move.nc", 417, 81));
_conditional_value_X3;})) {
                __right_value0 = (void*)0;
                __dec_obj4=prev_line,
                prev_line=(int* )come_increment_ref_count((int* )come_memdup(it2_6, "11move.nc", 418, 82, "int* "), "11move.nc", 418, 84);
                __dec_obj4 = come_decrement_ref_count(__dec_obj4, (void*)0, (void*)0, 0,0, (void*)0, "11move.nc", 418, 83);
                (it2_6 = come_decrement_ref_count(it2_6, (void*)0, (void*)0, 0, 0, (void*)0, "11move.nc", 419, 85));
                break;
            }
            (it2_6 = come_decrement_ref_count(it2_6, (void*)0, (void*)0, 0, 0, (void*)0, "11move.nc", 423, 86));
        }
        if(__right_value0 = (void*)0,
__right_value1 = (void*)0,
({(_conditional_value_X4=((line_is_unindented_function_top(((char* )(__right_value0=wstring_to_string(it))))&&line_is_function_signature(((char* )(__right_value1=wstring_to_string(prev_line)))))||line_is_method_top(((char* )(__right_value2=wstring_to_string(it))))));        (__right_value0 = come_decrement_ref_count(__right_value0, (void*)0, (void*)0, 1, 0, (void*)0, "11move.nc", 423, 87));
        (__right_value1 = come_decrement_ref_count(__right_value1, (void*)0, (void*)0, 1, 0, (void*)0, "11move.nc", 424, 88));
        (__right_value2 = come_decrement_ref_count(__right_value2, (void*)0, (void*)0, 1, 0, (void*)0, "11move.nc", 425, 89));
_conditional_value_X4;})) {
            ...
            ViWin_modifyOverCursorYValue(self);
            (prev_line = come_decrement_ref_count(prev_line, (void*)0, (void*)0, 0, 0, (void*)0, "11move.nc", 433, 90));
            break;
        }
        else if(__right_value0 = (void*)0,
({(_conditional_value_X5=(line_is_function_signature(((char* )(__right_value0=wstring_to_string(it))))));        (__right_value0 = come_decrement_ref_count(__right_value0, (void*)0, (void*)0, 1, 0, (void*)0, "11move.nc", 435, 91));
_conditional_value_X5;})) {
            __right_value0 = (void*)0;
            __right_value1 = (void*)0;
            next_line=(int* )come_increment_ref_count(list$1int$ph_item(self->texts,y+1,((int* )(__right_value0=__builtin_wstring("","11move.nc",436)))), "11move.nc", 436, 92);
            (__right_value0 = come_decrement_ref_count(__right_value0, (void*)0, (void*)0, 1, 0, (void*)0, "11move.nc", 436, 93));
            if(__right_value0 = (void*)0,
({(_conditional_value_X6=(line_is_function_top(((char* )(__right_value0=wstring_to_string(next_line))))));            (__right_value0 = come_decrement_ref_count(__right_value0, (void*)0, (void*)0, 1, 0, (void*)0, "11move.nc", 438, 94));
_conditional_value_X6;})) {
                ...
                ViWin_modifyUnderCursorYValue(self);
                ViWin_modifyOverCursorYValue(self);
                (next_line = come_decrement_ref_count(next_line, (void*)0, (void*)0, 0, 0, (void*)0, "11move.nc", 445, 95));
                (prev_line = come_decrement_ref_count(prev_line, (void*)0, (void*)0, 0, 0, (void*)0, "11move.nc", 445, 96));
                break;
            }
            (next_line = come_decrement_ref_count(next_line, (void*)0, (void*)0, 0, 0, (void*)0, "11move.nc", 449, 97));
        }
        it2++;
        (prev_line = come_decrement_ref_count(prev_line, (void*)0, (void*)0, 0, 0, (void*)0, "11move.nc", 451, 98));
    }
    come_call_finalizer(list$1int$ph$p_finalize, _o2_saved_5, (void*)0, (void*)0, 0, 0, 0, (void*)0, "11move.nc}", 453, 99);
    neo_current_frame = fr.prev;
}

void ViWin_gotoMethodTop(struct ViWin*  self  , struct Vi*  nvi  )
{
    struct neo_frame fr; fr.prev = neo_current_frame; fr.fun_name = "ViWin_gotoMethodTop"; fr.frame_id = ++neo_frame_id; neo_current_frame = &fr;
    int it2;
    void* __right_value0 = (void*)0;
    void* __right_value1 = (void*)0;
    ...
    memset(&next_line, 0, sizeof(next_line));
    it2=0;
    for(({(_conditional_value_X0=(_o2_saved_6=(struct list$1int$ph*)come_increment_ref_count(list$1int$ph_reverse(((struct list$1int$ph*)(__right_value0=list$1int$ph_sublist(self->texts,0,self->scroll+self->cursorY)))), "11move.nc", 456, 100),it=list$1int$ph_begin(_o2_saved_6)));    come_call_finalizer(list$1int$ph$p_finalize, __right_value0, (void*)0, (void*)0, 0, 1, 0, (void*)0, "11move.nc}", 456, 101);
_conditional_value_X0;})    ;({(_conditional_value_X1=(!list$1int$ph_end(_o2_saved_6)));_conditional_value_X1;});({(_conditional_value_X2=(it=list$1int$ph_next(_o2_saved_6)));_conditional_value_X2;})){
        y=self->cursorY+self->scroll-it2-1;
        if(__right_value0 = (void*)0,
({(_conditional_value_X3=(line_is_method_top(((char* )(__right_value0=wstring_to_string(it))))));        (__right_value0 = come_decrement_ref_count(__right_value0, (void*)0, (void*)0, 1, 0, (void*)0, "11move.nc", 459, 102));
_conditional_value_X3;})) {
            ViWin_saveReturnPoint(self);
            self->cursorX=0;
            self->cursorY=self->cursorY-it2-1;
...
({(_conditional_value_X4=(line_is_function_signature(((char* )(__right_value0=wstring_to_string(it))))));        (__right_value0 = come_decrement_ref_count(__right_value0, (void*)0, (void*)0, 1, 0, (void*)0, "11move.nc", 469, 103));
_conditional_value_X4;})) {
            __right_value0 = (void*)0;
            __right_value1 = (void*)0;
            next_line=(int* )come_increment_ref_count(list$1int$ph_item(self->texts,y+1,((int* )(__right_value0=__builtin_wstring("","11move.nc",470)))), "11move.nc", 470, 104);
            (__right_value0 = come_decrement_ref_count(__right_value0, (void*)0, (void*)0, 1, 0, (void*)0, "11move.nc", 470, 105));
            if(__right_value0 = (void*)0,
({(_conditional_value_X5=(line_is_function_top(((char* )(__right_value0=wstring_to_string(next_line))))));            (__right_value0 = come_decrement_ref_count(__right_value0, (void*)0, (void*)0, 1, 0, (void*)0, "11move.nc", 472, 106));
_conditional_value_X5;})) {
                ViWin_saveReturnPoint(self);
                self->cursorX=0;
                self->cursorY=self->cursorY-it2-1;
                ViWin_modifyUnderCursorYValue(self);
                ViWin_modifyOverCursorYValue(self);
                (next_line = come_decrement_ref_count(next_line, (void*)0, (void*)0, 0, 0, (void*)0, "11move.nc", 479, 107));
                break;
            }
            (next_line = come_decrement_ref_count(next_line, (void*)0, (void*)0, 0, 0, (void*)0, "11move.nc", 483, 108));
        }
        it2++;
    }
    come_call_finalizer(list$1int$ph$p_finalize, _o2_saved_6, (void*)0, (void*)0, 0, 0, 0, (void*)0, "11move.nc}", 487, 109);
    neo_current_frame = fr.prev;
}

void ViWin_gotoFunctionBottom(struct ViWin*  self  , struct Vi*  nvi  )
{
    struct neo_frame fr; fr.prev = neo_current_frame; fr.fun_name = "ViWin_gotoFunctionBottom"; fr.frame_id = ++neo_frame_id; neo_current_frame = &fr;
    int cursor_y;
    int it2;
    void* __right_value0 = (void*)0;
    ...
    memset(&it, 0, sizeof(it));
    cursor_y=self->scroll+self->cursorY+1;
    it2=0;
    for(({(_conditional_value_X0=(_o2_saved_7=(struct list$1int$ph*)come_increment_ref_count(list$1int$ph_sublist(self->texts,self->scroll+self->cursorY+1,-1), "11move.nc", 492, 110),it=list$1int$ph_begin(_o2_saved_7)));_conditional_value_X0;})    ;({(_conditional_value_X1=(!list$1int$ph_end(_o2_saved_7)));_conditional_value_X1;});({(_conditional_value_X2=(it=list$1int$ph_next(_o2_saved_7)));_conditional_value_X2;})){
        if(__right_value0 = (void*)0,
({(_conditional_value_X3=(line_is_function_bottom(((char* )(__right_value0=wstring_to_string(it))))));        (__right_value0 = come_decrement_ref_count(__right_value0, (void*)0, (void*)0, 1, 0, (void*)0, "11move.nc", 493, 111));
_conditional_value_X3;})) {
            ViWin_saveReturnPoint(self);
            self->cursorY+=it2+1;
            self->cursorX=0;
    ...
    come_call_finalizer(list$1int$ph$p_finalize, _o2_saved_7, (void*)0, (void*)0, 0, 0, 0, (void*)0, "11move.nc}", 508, 112);
    neo_current_frame = fr.prev;
}
...
static void lambda1(struct Vi*  self  , int key)
{
    ...
    ViWin_gotoBraceEnd_v11(self->activeWin,self);
...
static void lambda2(struct Vi*  self  , int key)
{
    ...
    switch (    key2) {
        case 91:
        ViWin_gotoFunctionTop(self->activeWin,self);
        ViWin_centeringCursor(self->activeWin);
        ViWin_saveInputedKeyOnTheMovingCursor_v14(self->activeWin);
        break;
        case 109:
        case 77:
        ViWin_gotoMethodTop(self->activeWin,self);
        ViWin_saveInputedKeyOnTheMovingCursor_v14(self->activeWin);
        break;
    }
    neo_current_frame = fr.prev;
...
static void lambda3(struct Vi*  self  , int key)
{
    ...
    switch (    key2) {
        case 93:
        ViWin_gotoFunctionBottom(self->activeWin,self);
        ViWin_centeringCursor(self->activeWin);
        ViWin_saveInputedKeyOnTheMovingCursor_v14(self->activeWin);
        break;
    }
...
struct Vi*  Vi_initialize_v11(struct Vi*  self  )
{
    struct neo_frame fr; fr.prev = neo_current_frame; fr.fun_name = "Vi_initialize_v11"; fr.frame_id = ++neo_frame_id; neo_current_frame = &fr;
    void* __right_value0 = (void*)0;
    struct Vi*  result  ;
    struct Vi*  __result_obj__0  ;
    memset(&result, 0, sizeof(result));
    result=(struct Vi* )come_increment_ref_count(Vi_initialize_v10((struct Vi* )come_increment_ref_count(self, "11move.nc", 510, 113)), "11move.nc", 510, 114);
    list$1lambda$_replace(result->events,37,lambda1);
    list$1lambda$_replace(result->events,91,lambda2);
    list$1lambda$_replace(result->events,93,lambda3);
        __result_obj__0 = (struct Vi* )come_increment_ref_count(result, "11move.nc", 547, 121);
    come_call_finalizer(Vi_finalize, self, (void*)0, (void*)0, 0, 0, 0, (void*)0, "11move.nc}", 547, 173);
    come_call_finalizer(Vi_finalize, result, (void*)0, (void*)0, 0, 0, 1, (void*)0, "11move.nc}", 547, 174);
    neo_current_frame = fr.prev;
    come_call_finalizer(Vi_finalize, __result_obj__0, (void*)0, (void*)0, 0, 0, 1, (void*)0, "11move.nc}", 547, 175);
    return __result_obj__0;
}
...
static struct list$1lambda$* list$1lambda$_push_back(struct list$1lambda$* self, void (*item)(struct Vi* ,int))
{
    ...
    if(self->len==0) {
        litem=(struct list_item$1lambda$*)come_increment_ref_count(((struct list_item$1lambda$*)(__right_value0=(struct list_item$1lambda$*)come_calloc(1, sizeof(struct list_item$1lambda$)*(1), "/usr/local/include/neo-c.h", 1678, 115, "struct list_item$1lambda$*"))), "/usr/local/include/neo-c.h", 1678, 116);
        litem->prev=((void*)0);
        litem->next=((void*)0);
        litem->item=item;
        self->tail=litem;
        ...
        __right_value0 = (void*)0;
        litem_7=(struct list_item$1lambda$*)come_increment_ref_count(((struct list_item$1lambda$*)(__right_value0=(struct list_item$1lambda$*)come_calloc(1, sizeof(struct list_item$1lambda$)*(1), "/usr/local/include/neo-c.h", 1688, 117, "struct list_item$1lambda$*"))), "/usr/local/include/neo-c.h", 1688, 118);
        ...
        __right_value0 = (void*)0;
        litem_8=(struct list_item$1lambda$*)come_increment_ref_count(((struct list_item$1lambda$*)(__right_value0=(struct list_item$1lambda$*)come_calloc(1, sizeof(struct list_item$1lambda$)*(1), "/usr/local/include/neo-c.h", 1698, 119, "struct list_item$1lambda$*"))), "/usr/local/include/neo-c.h", 1698, 120);
...
static void Vi_finalize(struct Vi*  self  )
{
    ...
    if(self!=((void*)0)&&self->wins!=((void*)0)) {
        come_call_finalizer(list$1ViWin$ph$p_finalize, self->wins, (void*)0, (void*)0, 0, 0, 0, (void*)0, "Vi_finalize}", 8, 156);
    }
    if(self!=((void*)0)&&self->events!=((void*)0)) {
        come_call_finalizer(list$1lambda$$p_finalize, self->events, (void*)0, (void*)0, 0, 0, 0, (void*)0, "Vi_finalize}", 2, 158);
    }
    if(self!=((void*)0)&&self->yank!=((void*)0)) {
        come_call_finalizer(list$1int$ph$p_finalize, self->yank, (void*)0, (void*)0, 0, 0, 0, (void*)0, "Vi_finalize}", 4, 159);
    }
    if(self!=((void*)0)&&self->fileYank!=((void*)0)) {
        come_call_finalizer(list$1int$ph$p_finalize, self->fileYank, (void*)0, (void*)0, 0, 0, 0, (void*)0, "Vi_finalize}", 5, 160);
    }
    if(self!=((void*)0)&&self->registers!=((void*)0)) {
        come_call_finalizer(map$2int$list$1int$ph$ph$p_finalize, self->registers, (void*)0, (void*)0, 0, 0, 0, (void*)0, "Vi_finalize}", 0, 165);
    }
    if(self!=((void*)0)&&self->registerKinds!=((void*)0)) {
        come_call_finalizer(map$2int$int$$p_finalize, self->registerKinds, (void*)0, (void*)0, 0, 0, 0, (void*)0, "Vi_finalize}", 0, 169);
    }
    if(self!=((void*)0)&&self->commandHistory!=((void*)0)) {
        come_call_finalizer(list$1char$ph$p_finalize, self->commandHistory, (void*)0, (void*)0, 0, 0, 0, (void*)0, "Vi_finalize}", 0, 172);
    }
        neo_current_frame = fr.prev;
}
...
static void list$1ViWin$ph$p_finalize(struct list$1ViWin$ph* self)
{
    ...
    while(it!=((void*)0)) {
        prev_it=it;
        it=it->next;
        come_call_finalizer(list_item$1ViWin$ph$p_finalize, prev_it, (void*)0, (void*)0, 0, 0, 0, (void*)0, "/usr/local/include/neo-c.h}", 1565, 155);
    }
            neo_current_frame = fr.prev;
}
...
static void list_item$1ViWin$ph$p_finalize(struct list_item$1ViWin$ph* self)
{
    ...
    if(self!=((void*)0)&&self->item!=((void*)0)) {
        come_call_finalizer(ViWin_finalize, self->item, (void*)0, (void*)0, 0, 0, 0, (void*)0, "list_item$1ViWin$ph$p_finalize}", 2, 154);
    }
            neo_current_frame = fr.prev;
}
...
static void ViWin_finalize(struct ViWin*  self  )
{
    ...
    if(self!=((void*)0)&&self->texts!=((void*)0)) {
        come_call_finalizer(list$1int$ph$p_finalize, self->texts, (void*)0, (void*)0, 0, 0, 0, (void*)0, "ViWin_finalize}", 2, 122);
    }
    if(self!=((void*)0)&&self->texts_length!=((void*)0)) {
        come_call_finalizer(list$1int$$p_finalize, self->texts_length, (void*)0, (void*)0, 0, 0, 0, (void*)0, "ViWin_finalize}", 18, 124);
    }
    if(self!=((void*)0)&&self->returnPoint!=((void*)0)) {
        come_call_finalizer(tuple3$3int$int$int$$p_finalize, self->returnPoint, (void*)0, (void*)0, 0, 0, 0, (void*)0, "ViWin_finalize}", 4, 125);
    }
    if(self!=((void*)0)&&self->returnPointStack!=((void*)0)) {
        come_call_finalizer(list$1tuple3$3int$int$int$$ph$p_finalize, self->returnPointStack, (void*)0, (void*)0, 0, 0, 0, (void*)0, "ViWin_finalize}", 0, 128);
    }
    if(self!=((void*)0)&&self->undo!=((void*)0)) {
        come_call_finalizer(list$1list$1int$ph$ph$p_finalize, self->undo, (void*)0, (void*)0, 0, 0, 0, (void*)0, "ViWin_finalize}", 2, 131);
    }
    if(self!=((void*)0)&&self->undoScroll!=((void*)0)) {
        come_call_finalizer(list$1int$$p_finalize, self->undoScroll, (void*)0, (void*)0, 0, 0, 0, (void*)0, "ViWin_finalize}", 7, 132);
    }
    if(self!=((void*)0)&&self->undoCursorX!=((void*)0)) {
        come_call_finalizer(list$1int$$p_finalize, self->undoCursorX, (void*)0, (void*)0, 0, 0, 0, (void*)0, "ViWin_finalize}", 8, 133);
    }
    if(self!=((void*)0)&&self->undoCursorY!=((void*)0)) {
        come_call_finalizer(list$1int$$p_finalize, self->undoCursorY, (void*)0, (void*)0, 0, 0, 0, (void*)0, "ViWin_finalize}", 9, 134);
    }
    if(self!=((void*)0)&&self->fileName!=((void*)0)) {
        (self->fileName = come_decrement_ref_count(self->fileName, (void*)0, (void*)0, 0, 0, (void*)0, "ViWin_finalize", 10, 135));
    }
    if(self!=((void*)0)&&self->inputedKeys!=((void*)0)) {
        come_call_finalizer(list$1int$$p_finalize, self->inputedKeys, (void*)0, (void*)0, 0, 0, 0, (void*)0, "ViWin_finalize}", 11, 136);
    }
    if(self!=((void*)0)&&self->savedInputedKeys!=((void*)0)) {
        come_call_finalizer(list$1int$$p_finalize, self->savedInputedKeys, (void*)0, (void*)0, 0, 0, 0, (void*)0, "ViWin_finalize}", 12, 137);
    }
    if(self!=((void*)0)&&self->macro!=((void*)0)) {
        come_call_finalizer(map$2int$list$1list$1int$$ph$ph$p_finalize, self->macro, (void*)0, (void*)0, 0, 0, 0, (void*)0, "ViWin_finalize}", 2, 144);
    }
    if(self!=((void*)0)&&self->recordingMacro!=((void*)0)) {
        come_call_finalizer(list$1list$1int$$ph$p_finalize, self->recordingMacro, (void*)0, (void*)0, 0, 0, 0, (void*)0, "ViWin_finalize}", 14, 145);
    }
    if(self!=((void*)0)&&self->runningMacro!=((void*)0)) {
        come_call_finalizer(list$1list$1int$$ph$p_finalize, self->runningMacro, (void*)0, (void*)0, 0, 0, 0, (void*)0, "ViWin_finalize}", 15, 146);
    }
    if(self!=((void*)0)&&self->mark!=((void*)0)) {
        come_call_finalizer(map$2int$tuple3$3int$int$int$$ph$p_finalize, self->mark, (void*)0, (void*)0, 0, 0, 0, (void*)0, "ViWin_finalize}", 2, 151);
    }
    if(self!=((void*)0)&&self->completionCandidates!=((void*)0)) {
        come_call_finalizer(list$1int$ph$p_finalize, self->completionCandidates, (void*)0, (void*)0, 0, 0, 0, (void*)0, "ViWin_finalize}", 17, 152);
    }
    if(self!=((void*)0)&&self->completionPrefix!=((void*)0)) {
        (self->completionPrefix = come_decrement_ref_count(self->completionPrefix, (void*)0, (void*)0, 0, 0, (void*)0, "ViWin_finalize", 18, 153));
    }
            neo_current_frame = fr.prev;
}
...
static void list$1int$$p_finalize(struct list$1int$* self)
{
    ...
    while(it!=((void*)0)) {
        prev_it=it;
        it=it->next;
        come_call_finalizer(list_item$1int$$p_finalize, prev_it, (void*)0, (void*)0, 0, 0, 0, (void*)0, "/usr/local/include/neo-c.h}", 1565, 123);
    }
            neo_current_frame = fr.prev;
}
...
static void list$1tuple3$3int$int$int$$ph$p_finalize(struct list$1tuple3$3int$int$int$$ph* self)
{
    ...
    while(it!=((void*)0)) {
        prev_it=it;
        it=it->next;
        come_call_finalizer(list_item$1tuple3$3int$int$int$$ph$p_finalize, prev_it, (void*)0, (void*)0, 0, 0, 0, (void*)0, "/usr/local/include/neo-c.h}", 1565, 127);
    }
            neo_current_frame = fr.prev;
}
...
static void list_item$1tuple3$3int$int$int$$ph$p_finalize(struct list_item$1tuple3$3int$int$int$$ph* self)
{
    ...
    if(self!=((void*)0)&&self->item!=((void*)0)) {
        come_call_finalizer(tuple3$3int$int$int$$p_finalize, self->item, (void*)0, (void*)0, 0, 0, 0, (void*)0, "list_item$1tuple3$3int$int$int$$ph$p_finalize}", 2, 126);
    }
            neo_current_frame = fr.prev;
}
...
static void list$1list$1int$ph$ph$p_finalize(struct list$1list$1int$ph$ph* self)
{
    ...
    while(it!=((void*)0)) {
        prev_it=it;
        it=it->next;
        come_call_finalizer(list_item$1list$1int$ph$ph$p_finalize, prev_it, (void*)0, (void*)0, 0, 0, 0, (void*)0, "/usr/local/include/neo-c.h}", 1565, 130);
    }
            neo_current_frame = fr.prev;
}
...
static void list_item$1list$1int$ph$ph$p_finalize(struct list_item$1list$1int$ph$ph* self)
{
    ...
    if(self!=((void*)0)&&self->item!=((void*)0)) {
        come_call_finalizer(list$1int$ph$p_finalize, self->item, (void*)0, (void*)0, 0, 0, 0, (void*)0, "list_item$1list$1int$ph$ph$p_finalize}", 2, 129);
    }
            neo_current_frame = fr.prev;
}
...
static void map$2int$list$1list$1int$$ph$ph$p_finalize(struct map$2int$list$1list$1int$$ph$ph* self)
{
    ...
    for(i=0    ;i<self->size;i++){
        if(self->item_existance[i]) {
            if(1) {
                come_call_finalizer(list$1list$1int$$ph$p_finalize, self->items[i], (void*)0, (void*)0, 0, 0, 0, (void*)0, "/usr/local/include/neo-c.h}", 2, 140);
            }
        }
    }
    ...
    come_call_finalizer(list$1int$$p_finalize, self->key_list, (void*)0, (void*)0, 0, 0, 0, (void*)0, "/usr/local/include/neo-c.h}", 3524, 141);
    (self->hashes = come_decrement_ref_count(self->hashes, (void*)0, (void*)0, 0, 0, (void*)0, "/usr/local/include/neo-c.h", 3526, 142));
    (self->item_existance = come_decrement_ref_count(self->item_existance, (void*)0, (void*)0, 0, 0, (void*)0, "/usr/local/include/neo-c.h", 3527, 143));
            neo_current_frame = fr.prev;
}
...
static void list$1list$1int$$ph$p_finalize(struct list$1list$1int$$ph* self)
{
    ...
    while(it!=((void*)0)) {
        prev_it=it;
        it=it->next;
        come_call_finalizer(list_item$1list$1int$$ph$p_finalize, prev_it, (void*)0, (void*)0, 0, 0, 0, (void*)0, "/usr/local/include/neo-c.h}", 1565, 139);
    }
                    neo_current_frame = fr.prev;
}
...
static void list_item$1list$1int$$ph$p_finalize(struct list_item$1list$1int$$ph* self)
{
    ...
    if(self!=((void*)0)&&self->item!=((void*)0)) {
        come_call_finalizer(list$1int$$p_finalize, self->item, (void*)0, (void*)0, 0, 0, 0, (void*)0, "list_item$1list$1int$$ph$p_finalize}", 2, 138);
    }
            neo_current_frame = fr.prev;
}
...
static void map$2int$tuple3$3int$int$int$$ph$p_finalize(struct map$2int$tuple3$3int$int$int$$ph* self)
{
    ...
    for(i=0    ;i<self->size;i++){
        if(self->item_existance[i]) {
            if(1) {
                come_call_finalizer(tuple3$3int$int$int$$p_finalize, self->items[i], (void*)0, (void*)0, 0, 0, 0, (void*)0, "/usr/local/include/neo-c.h}", 3509, 147);
            }
        }
    }
    come_free((char*)self->items);
    ...
    come_free((char*)self->keys);
    come_call_finalizer(list$1int$$p_finalize, self->key_list, (void*)0, (void*)0, 0, 0, 0, (void*)0, "/usr/local/include/neo-c.h}", 3524, 148);
    (self->hashes = come_decrement_ref_count(self->hashes, (void*)0, (void*)0, 0, 0, (void*)0, "/usr/local/include/neo-c.h", 3526, 149));
    (self->item_existance = come_decrement_ref_count(self->item_existance, (void*)0, (void*)0, 0, 0, (void*)0, "/usr/local/include/neo-c.h", 3527, 150));
            neo_current_frame = fr.prev;
}
...
static void list$1lambda$$p_finalize(struct list$1lambda$* self)
{
    ...
    while(it!=((void*)0)) {
        prev_it=it;
        it=it->next;
        come_call_finalizer(list_item$1lambda$$p_finalize, prev_it, (void*)0, (void*)0, 0, 0, 0, (void*)0, "/usr/local/include/neo-c.h}", 1565, 157);
    }
            neo_current_frame = fr.prev;
}
...
static void map$2int$list$1int$ph$ph$p_finalize(struct map$2int$list$1int$ph$ph* self)
{
    ...
    for(i=0    ;i<self->size;i++){
        if(self->item_existance[i]) {
            if(1) {
                come_call_finalizer(list$1int$ph$p_finalize, self->items[i], (void*)0, (void*)0, 0, 0, 0, (void*)0, "/usr/local/include/neo-c.h}", 3509, 161);
            }
        }
    }
    come_free((char*)self->items);
    ...
    come_free((char*)self->keys);
    come_call_finalizer(list$1int$$p_finalize, self->key_list, (void*)0, (void*)0, 0, 0, 0, (void*)0, "/usr/local/include/neo-c.h}", 3524, 162);
    (self->hashes = come_decrement_ref_count(self->hashes, (void*)0, (void*)0, 0, 0, (void*)0, "/usr/local/include/neo-c.h", 3526, 163));
    (self->item_existance = come_decrement_ref_count(self->item_existance, (void*)0, (void*)0, 0, 0, (void*)0, "/usr/local/include/neo-c.h", 3527, 164));
            neo_current_frame = fr.prev;
}
...
static void map$2int$int$$p_finalize(struct map$2int$int$* self)
{
    ...
    come_free((char*)self->keys);
    come_call_finalizer(list$1int$$p_finalize, self->key_list, (void*)0, (void*)0, 0, 0, 0, (void*)0, "/usr/local/include/neo-c.h}", 3524, 166);
    (self->hashes = come_decrement_ref_count(self->hashes, (void*)0, (void*)0, 0, 0, (void*)0, "/usr/local/include/neo-c.h", 3526, 167));
    (self->item_existance = come_decrement_ref_count(self->item_existance, (void*)0, (void*)0, 0, 0, (void*)0, "/usr/local/include/neo-c.h", 3527, 168));
            neo_current_frame = fr.prev;
}
...
static void list$1char$ph$p_finalize(struct list$1char$ph* self)
{
    ...
    while(it!=((void*)0)) {
        prev_it=it;
        it=it->next;
        come_call_finalizer(list_item$1char$ph$p_finalize, prev_it, (void*)0, (void*)0, 0, 0, 0, (void*)0, "/usr/local/include/neo-c.h}", 1565, 171);
    }
            neo_current_frame = fr.prev;
}
...
static void list_item$1char$ph$p_finalize(struct list_item$1char$ph* self)
{
    ...
    if(self!=((void*)0)&&self->item!=((void*)0)) {
        (self->item = come_decrement_ref_count(self->item, (void*)0, (void*)0, 0, 0, (void*)0, "list_item$1char$ph$p_finalize", 2, 170));
    }
            neo_current_frame = fr.prev;
}
