...
void come_heap_final()
{
    ...
    if(gComeDebugLib) {
        ...
        while(it_2) {
            n_3++;
            if(it_2->class_name) {
                printf("#%d %p (%s) %s %d\n",n_3,(char*)it_2+sizeof(struct sMemHeader)+sizeof(unsigned long  int)+sizeof(unsigned long  int),it_2->class_name,it_2->sname,it_2->sline);
            }
            it_2=it_2->next;
        }
        if(n_3>0) {
