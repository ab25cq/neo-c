...
struct sNode* string_node_v9(char* buf, char* head, int head_sline, struct sInfo* info)
{
    ...
    if(charp_operator_equals(buf,"while")) {
        ...
        _inf_value1=(struct sNode*)come_calloc(1, sizeof(struct sNode), "09while.c", 70, "struct sNode");
        _inf_obj_value1=(struct sWhileNode*)come_increment_ref_count(((struct sWhileNode*)(__right_value1=sWhileNode_initialize((struct sWhileNode*)come_increment_ref_count((struct sWhileNode*)come_calloc(1, sizeof(struct sWhileNode)*(1), "09while.c", 70, "struct sWhileNode*")),(struct sNode*)come_increment_ref_count(expression_node),block,info))));
