...
int main()
{
    ...
    memset(&p, 0, sizeof(p));
    # 11 "/tmp/test_remove_proc4.nc"
    xs=(struct list$1proc$p*)come_increment_ref_count(list$1proc$p_initialize((struct list$1proc$p*)come_increment_ref_count((struct list$1proc$p*)come_calloc(1, sizeof(struct list$1proc$p)*(1), "/tmp/test_remove_proc4.nc", 11, 1, "struct list$1proc$p*"), "/tmp/test_remove_proc4.nc", 11, 6)), "/tmp/test_remove_proc4.nc", 11, 7);
    # 12 "/tmp/test_remove_proc4.nc"
    p=((void*)0);
    # 13 "/tmp/test_remove_proc4.nc"
    list$1proc$p_remove(xs,p,(_Bool)0);
    # 14 "/tmp/test_remove_proc4.nc"
    list$1proc$p_remove(xs,p,(_Bool)1);
    # 15 "/tmp/test_remove_proc4.nc"
        __result_obj__0 = 0;
    come_call_finalizer(list$1proc$p$p_finalize, xs, (void*)0, (void*)0, 0, 0, 0, (void*)0, "/tmp/test_remove_proc4.nc}", 15, 12);
...
static _Bool proc_equals(struct proc*  left  , struct proc*  right  )
{
    ...
    # 1 "proc_equals"
    if(left->pagetable!=right->pagetable) {
        # 0 "proc_equals"
                neo_current_frame = fr.prev;
        return (_Bool)0;
    }
