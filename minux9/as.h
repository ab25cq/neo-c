...
// as.h (place near existing arg_parser group)
...
struct rawdata {
    ...
    struct sectionpos position;
    size_t line;
    struct symbol *sym; // For .quad <symbol>: symbol to resolve
    int is_abs64;       // If 1, output 8B as an absolute address
    long addend;        // For future extensions (fixed at 0 for now)
};
