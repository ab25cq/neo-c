...
static void print_shdrs_and_sym(const unsigned char* buf, const Elf64_Ehdr* eh) {
  ...
  for (uint16_t i=0;i<eh->e_shnum;i++) {
    ...
    if (sh->sh_type == SHT_STRTAB && i != eh->e_shstrndx) {
      // Usually this is .strtab
      // (If .dynstr exists there can be multiple, but assume the first in this minimal implementation)
      if (idx_str == -1) idx_str = i;
    }
  }

  // Find the __global_pointer$ location (needed by exec in main.c)
  if (idx_sym >= 0) {
    const Elf64_Shdr* sh_sym = &sh_base[idx_sym];
    // Use the linked string table if possible
...
// Read whole file in small slices so we never exceed the 4KB per-read limit
// of the MINUX9 kernel (Sys_read panics if asked for more).
