...
int main()
{
    ...
    memset(&p, 0, sizeof(p));
    # 11 "/tmp/test_remove_proc.nc"
    xs=(struct list$1proc$p*)come_increment_ref_count(list$1proc$p_initialize((struct list$1proc$p*)come_increment_ref_count((struct list$1proc$p*)come_calloc(1, sizeof(struct list$1proc$p)*(1), "/tmp/test_remove_proc.nc", 11, 1, "struct list$1proc$p*"), "/tmp/test_remove_proc.nc", 11, 6)), "/tmp/test_remove_proc.nc", 11, 7);
    # 12 "/tmp/test_remove_proc.nc"
    p=((void*)0);
    # 13 "/tmp/test_remove_proc.nc"
    list$1proc$p_remove(xs,p,(_Bool)0);
    # 14 "/tmp/test_remove_proc.nc"
        __result_obj__0 = 0;
    come_call_finalizer(list$1proc$p$p_finalize, xs, (void*)0, (void*)0, 0, 0, 0, (void*)0, "/tmp/test_remove_proc.nc}", 14, 12);
