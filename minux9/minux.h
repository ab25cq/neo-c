...
#define NINDIRECT  (BSIZE / sizeof(uint32_t))  // Entries per indirect block (512/4=128)
#define MAXFILE    (NDIRECT + NINDIRECT + NINDIRECT * NINDIRECT)

#define ROOTINO    1     // i-node number of the root directory
#define T_DIR      1
#define T_FILE     2
#define T_SYMLINK  3
...
#define DIRSIZ     14    // Maximum length of a directory entry name

// ───────────────────────────────────────────────────────────────────────
// On-disk structures
// ────────────────────────────────────────────────────────────────────────

// Superblock (placed at block number 1)
struct superblock {
    uint32_t size;       // Total number of blocks
    uint32_t nblocks;    // Number of data blocks
    uint32_t ninodes;    // Number of inodes
    uint32_t nlog;       // Number of log blocks (0 if unused)
    uint32_t logstart;   // Starting block of the log area (0 if unused)
    uint32_t inodestart; // Starting block of the inode area
    uint32_t bmapstart;  // Starting block of the bitmap
};

// On-disk i-node structure
struct dinode {
    uint16_t type;             // 0=free, 1=directory, 2=file, 3=symlink
    uint16_t major;            // Device major number (0 if unused)
    uint16_t minor;            // Device minor number (0 if unused)
    uint16_t nlink;            // Hard link count
    uint32_t size;             // File size in bytes
    uint32_t mode;             // Permission/type bits (simplified)
    uint16_t uid;              // Owner
    uint16_t gid;              // Group
    uint32_t atime;            // Last access time
    uint32_t mtime;            // Last modification time
    uint32_t ctime;            // Metadata change time
    /* Direct pointers [0..NDIRECT-1], single indirect = addrs[NDIRECT], double indirect = addrs[NDIRECT+1] */
    uint32_t addrs[NDIRECT + 2];
};

// Directory entry
struct dirent {
    uint16_t inum;           // File/directory i-node number
    char     name[DIRSIZ];   // File name (fixed 14 bytes, no NUL terminator)
};

// Simplified stat structure (shared by user and kernel)
struct stat {
...
// Low-level block read (expected to be implemented externally)
// blockno: block number on the filesystem (starting at 0)
// buf:     buffer allocated for BSIZE bytes
...
// Read an i-node
// inum: i-node number (1 through ninodes)
// dest: buffer for struct dinode (allocated by caller)
void read_inode(uint32_t inum, struct dinode *dest);

// Read file (or directory) data into a buffer
// inode: must be populated with read_inode beforehand
// offset, size: offset and number of bytes to read
// buf: destination buffer (allocate at least size bytes)
void read_data(struct dinode *inode, uint32_t offset, uint8_t *buf, uint32_t size);

// Lookup a filename from the root directory
// path: given as an absolute path like "/foo/bar.txt"
// return: inode number if found, otherwise 0
uint32_t path_lookup(const char *path);

// Find an entry inside a directory
// parent: dinode structure of the parent directory
// name:   target filename (NUL-terminated string)
// return: inode number, or 0 if not found
uint32_t dir_lookup(struct dinode *parent, const char *name);

// Load an i-node and print its contents (for debugging)
...
#define read(fd, buf, len) ({                                      \
    ...
    /* Execute ecall; return value comes back in a0 */             \
    asm volatile("ecall"                                           \
                 : "+r"(_a0)                                       \
                 : "r"(_a1), "r"(_a2), "r"(_a7)                     \
                 : "memory");                                       \
    /* Save a0 locally and use it as the statement-expression value */ \
    _ret = _a0;                                                    \
    _ret;                                                          \
})
...
// Version that captures and returns the result
#define fork()                                        \
    ({                                                    \
