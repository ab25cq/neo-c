...
#define MSTATUS_TSR  (1UL << 22) // Define TSR bit (22)
...
// Read mtime value (directly from the CLINT address)
static inline uint64_t read_clint_mtime(void) {
    return *(volatile uint64_t *)CLINT_MTIME;
}
    
// Write mtimecmp value (directly to the CLINT address)
...
void start()
{
    ...
    x &= ~(1UL << 20);     /// TVM clear
    x &= ~(MSTATUS_TSR);   // Clear TSR so sret and sfence can be used
    w_mstatus(x);
