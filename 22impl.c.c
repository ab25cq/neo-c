...
memset(&header_66, 0, sizeof(struct buffer*));
    if(charp_operator_equals(buf,"impl")) {
        ...
        word_48=(char*)come_increment_ref_count(parse_word(info));
        if(*info->p==60) {
            info->p++;
            skip_spaces_and_lf(info);
            list$1charph_reset(info->generics_type_names);
            while((_Bool)1) {
                generics_name_51=(char*)come_increment_ref_count(parse_word(info));
        ...
        info->impl_type=(struct sType*)come_increment_ref_count(sType_initialize((struct sType*)come_increment_ref_count(((struct sType*)(__right_value85=(struct sType*)come_calloc(1, sizeof(struct sType)*(1), "22impl.c", 45, "sType")))),word_48,(_Bool)0,info));
        ...
        while(*info->p!=125) {
            parse_sharp_v5(info);
            head_62=info->p;
            buf_63=(char*)come_increment_ref_count(parse_word(info));
            parse_sharp_v5(info);
            node_64=(struct sNode*)come_increment_ref_count(top_level_v99(buf_63,head_62,head_sline,info));
            parse_sharp_v5(info);
            while(*info->p==59) {
                info->p++;
                skip_spaces_and_lf(info);
        ...
        header_66=(struct buffer*)come_increment_ref_count(buffer_initialize((struct buffer*)come_increment_ref_count(((struct buffer*)(__right_value89=(struct buffer*)come_calloc(1, sizeof(struct buffer)*(1), "22impl.c", 82, "buffer"))))));
