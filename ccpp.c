...
{
    memset(&opts_global, 0, sizeof(opts_global));
    ...
    for (int i=1; i<global_argc; ++i) {
        ...
        if (a[0]=='-' && a[1]=='I') {
            const char *dir = NULL;
            if (a[2] != '\0') dir = a+2; else if (i+1 < global_argc) { dir = global_argv[++i]; }
            if (dir) {
            ...
            const char *def = a+2;
            if (*def == '\0' && i+1 < global_argc) def = global_argv[++i];
            if (def && *def) {
            ...
            const char *name = a+2;
            if (*name == '\0' && i+1 < global_argc) name = global_argv[++i];
            if (name && *name) {
                // Accept forms like "-U__GNUC__-v" by trimming the macro name
                // to the leading identifier portion.
                char idbuf[256]; size_t j=0; const char *p=name;
