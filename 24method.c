...
;    struct sType* type  =0;
    ...
    if(parse_method_generics_type&&*info->p==60) {
        ...
        while(1) {
            if(*info->p==0) {
                ...
                multiple_assign_var15=(come_push_stackframe("24method.nc", 962, 1),__exception_result_var_b2=((struct tuple3$3sType$phchar$ph_Bool$*)(__right_value0=parse_type(info,0,0,0))), come_pop_stackframe(), __exception_result_var_b2);
    ...
    if(*info->p!=123) {
        (come_push_stackframe("24method.nc", 975, 2),__exception_result_var_b3=expected_next_character(40,info), come_pop_stackframe(), __exception_result_var_b3);
        while(1) {
            if(*info->p==41) {
                info->p++;
                skip_spaces_and_lf(info);
