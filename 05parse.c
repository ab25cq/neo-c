...
int err_msg2(struct sInfo* info  , char* msg, ...)
{
    ...
    if(!info->no_output_come_code) {
        ...
        vasprintf(&msg2,msg,args);
        __builtin_va_end(args);
        buf=(struct buffer*)come_increment_ref_count(buffer_initialize((struct buffer*)come_increment_ref_count((struct buffer*)come_calloc(1, sizeof(struct buffer)*(1), "05parse.nc", 59, "struct buffer*"))));
        if(info->come_fun) {
            buffer_append_format(buf,"%s %d(%d): %s in fun(%s)",info->sname,info->sline,info->sline_real,msg2,info->come_fun->mName);
        }
        else {
...
char* parse_word(_Bool digits, struct sInfo* info  )
{
    ...
    buf=(struct buffer*)come_increment_ref_count(buffer_initialize((struct buffer*)come_increment_ref_count((struct buffer*)come_calloc(1, sizeof(struct buffer)*(1), "05parse.nc", 98, "struct buffer*"))));
    parse_sharp_v5(info);
    if(digits) {
...
char* backtrace_parse_word(struct sInfo* info  )
{
    ...
    if(xisalpha(*info->p)||*info->p==95) {
        __dec_obj1=buf,
        buf=(char*)come_increment_ref_count((come_push_stackframe("05parse.nc", 144, 2),__exception_result_var_b3=(come_push_stackframe("05parse.nc", 144, 0),__exception_result_var_b1=parse_word(0,info), come_pop_stackframe(), __exception_result_var_b1), come_pop_stackframe(), __exception_result_var_b3));
    ...
    while(1) {
        if(*info->p==35) {
            ...
            if(parsecmp("pragma",info)) {
                buf=(struct buffer*)come_increment_ref_count(buffer_initialize((struct buffer*)come_increment_ref_count((struct buffer*)come_calloc(1, sizeof(struct buffer)*(1), "05parse.nc", 323, "struct buffer*"))));
...
_conditional_value_X0;})) {
                ...
                skip_spaces_and_tabs(info);
                line=0;
                fname=(struct buffer*)come_increment_ref_count(buffer_initialize((struct buffer*)come_increment_ref_count((struct buffer*)come_calloc(1, sizeof(struct buffer)*(1), "05parse.nc", 349, "struct buffer*"))));
                if(!xisdigit(*info->p)) {
                ...
                line_0=0;
                fname_1=(struct buffer*)come_increment_ref_count(buffer_initialize((struct buffer*)come_increment_ref_count((struct buffer*)come_calloc(1, sizeof(struct buffer)*(1), "05parse.nc", 399, "struct buffer*"))));
                while(xisdigit(*info->p)) {
