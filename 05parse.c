...
static int output_compile_message(struct sInfo*  info  , const char* level, const char* msg2)
{
    ...
    if((info->end-info->p)>30&&(info->p-info->head)>30) {
        ...
        ({ long long __neo_array_index9_0 = (long long)(60); if(__neo_array_index9_0 < 0 || __neo_array_index9_0 >= (long long)(61)) { extern void stackframe(void) __attribute__((weak)); extern void exit(int); puts("array index out of bounds"); if(stackframe) stackframe(); else { puts("stackframe"); puts(__func__); } exit(2); } mem[__neo_array_index9_0] = 0; });
        printf("%s\n",mem);
    }
    printf("\n");
        neo_current_frame = fr.prev;
