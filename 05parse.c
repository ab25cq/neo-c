...
int err_msg(struct sInfo* info  , char* msg, ...)
{
    ...
    if(!info->no_output_come_code) {
        ...
        if(info->come_fun) {
            buffer_append_format(buf,"%s %d: %s in fun(%s)",info->sname,info->sline,msg2,info->come_fun->mName);
        }
        else {
            buffer_append_format(buf,"%s %d: %s",info->sname,info->sline,msg2);
        }
        info->err_num++;
        free(msg2);
    ...
    while(1) {
        if(*info->p==35) {
            ...
            if(parsecmp("pragma",info)) {
                buf=(struct buffer*)come_increment_ref_count(buffer_initialize((struct buffer*)come_increment_ref_count((struct buffer*)come_calloc(1, sizeof(struct buffer)*(1), "05parse.nc", 322, "struct buffer*"))));
...
_conditional_value_X0;})) {
                ...
                skip_spaces_and_tabs(info);
                line=0;
                fname=(struct buffer*)come_increment_ref_count(buffer_initialize((struct buffer*)come_increment_ref_count((struct buffer*)come_calloc(1, sizeof(struct buffer)*(1), "05parse.nc", 348, "struct buffer*"))));
                if(!xisdigit(*info->p)) {
                ...
                line_0=0;
                fname_1=(struct buffer*)come_increment_ref_count(buffer_initialize((struct buffer*)come_increment_ref_count((struct buffer*)come_calloc(1, sizeof(struct buffer)*(1), "05parse.nc", 398, "struct buffer*"))));
                while(xisdigit(*info->p)) {
