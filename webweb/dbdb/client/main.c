...
void fun_block1_mainnc(struct __current_stack1__* parent, int it  , _Bool* it2);
...
int main(int argc, char** argv)
{
    ...
    __current_stack1__.n = &n;
    })    ,    client_socket(3366,"127.0.0.1",&__current_stack1__,(void*)fun_block1_mainnc);
come_heap_final();
}
...
void fun_block1_mainnc(struct __current_stack1__* parent, int it  , _Bool* it2)
{
    ...
    char buf[1024]={0};
    message=messages[(*(parent->n))];
    puts(message);
    write(it,message,strlen(message));
    size=read(it,buf,1023);
    buf[size]=0;
    printf("%s",buf);
    (*(parent->n))++;
    if(messages[(*(parent->n))]==((void*)0)) {
        *it2=1;
        return;
    }
}
...
int httpsd_socket(int port, _Bool reuse, void* parent, void (*block)(void*,struct ssl_st*,_Bool*))
{
    ...
    memset(&addr_56, 0, sizeof(addr_56));
    ...
    while(1) {
        len=sizeof(addr_56);
        client=accept(sock,(struct sockaddr*)&addr_56,&len);
        if(client<0) {
        ...
        if(SSL_accept(ssl)<=0) {
            ...
            if(break_) {
                come_call_finalizer(sockaddr_in_finalize, (&addr_56), (void*)0, (void*)0, 1, 0, 0, (void*)0);
                break;
            }
        }
        SSL_shutdown(ssl);
