...
static void test_function_pointer_pointer();
static void test_pointer_to_array_of_fnptr();
static void test_pointer_of_pointer_array();
int main();
...
static void test_function_pointer_pointer()
{
    ...
    do {
        if(!(((*p))(3)==4)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","(*p)(3) == 4","code/c_test_compat36.nc",24);
            g_ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(((**pp))(10)==11)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","(**pp)(10) == 11","code/c_test_compat36.nc",25);
            g_ok=(_Bool)0;
        }
    } while(0);
    p+=1;
    do {
        if(!(((*p))(7)==6)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","(*p)(7) == 6","code/c_test_compat36.nc",27);
            g_ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(pp[0]==&ops[1])) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","pp[0] == &ops[1]","code/c_test_compat36.nc",28);
            g_ok=(_Bool)0;
        }
    } while(0);
}
...
static void test_pointer_to_array_of_fnptr()
{
    ...
    do {
        if(!(a&&*a==5)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","a && *a == 5","code/c_test_compat36.nc",38);
            g_ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(b&&*b==6)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","b && *b == 6","code/c_test_compat36.nc",39);
            g_ok=(_Bool)0;
        }
    } while(0);
}
...
static void test_pointer_of_pointer_array()
{
    ...
    do {
        if(!(*pp[0]==1)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","*pp[0] == 1","code/c_test_compat36.nc",47);
            g_ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(*pp[1]==2)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","*pp[1] == 2","code/c_test_compat36.nc",48);
            g_ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(*pp[2]==3)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","*pp[2] == 3","code/c_test_compat36.nc",49);
            g_ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(&pp[2]-&pp[0]==2)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","&pp[2] - &pp[0] == 2","code/c_test_compat36.nc",50);
            g_ok=(_Bool)0;
        }
    } while(0);
    end=plist+3;
    count=(unsigned long )(end-pp);
    do {
        if(!(count==3)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","count == 3","code/c_test_compat36.nc",54);
            g_ok=(_Bool)0;
        }
    } while(0);
}
