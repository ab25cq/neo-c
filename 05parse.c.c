...
int err_msg(struct sInfo* info, char* msg, ...)
{
    ...
    if(!info->no_output_come_code) {
        ...
        buf=(struct buffer*)come_increment_ref_count(buffer_initialize((struct buffer*)come_increment_ref_count((struct buffer*)come_calloc(1, sizeof(struct buffer)*(1), "05parse.c", 38, "struct buffer*"))));
        if(last_lf) {
            col=info->p-last_lf;
...
int err_msg2(struct sInfo* info, char* msg, ...)
{
    ...
    if(!info->no_output_come_code) {
        ...
        buf=(struct buffer*)come_increment_ref_count(buffer_initialize((struct buffer*)come_increment_ref_count((struct buffer*)come_calloc(1, sizeof(struct buffer)*(1), "05parse.c", 83, "struct buffer*"))));
        if(last_lf) {
            col=info->p-last_lf;
            buffer_append_format(buf,"%s %d(real %d)(block %d) %d: %s",info->sname,info->sline,info->sline_real,info->sline_block,col,msg2);
        }
...
char* parse_word(_Bool digits, struct sInfo* info)
{
    ...
    buf=(struct buffer*)come_increment_ref_count(buffer_initialize((struct buffer*)come_increment_ref_count((struct buffer*)come_calloc(1, sizeof(struct buffer)*(1), "05parse.c", 126, "struct buffer*"))));
    parse_sharp_v5(info);
    if(digits) {
    ...
    while(1) {
        if(*info->p==35) {
            ...
            if(parsecmp("pragma",info)) {
                buf=(struct buffer*)come_increment_ref_count(buffer_initialize((struct buffer*)come_increment_ref_count((struct buffer*)come_calloc(1, sizeof(struct buffer)*(1), "05parse.c", 350, "struct buffer*"))));
...
_conditional_value_X0;})) {
                ...
                skip_spaces_and_tabs(info);
                line=0;
                fname=(struct buffer*)come_increment_ref_count(buffer_initialize((struct buffer*)come_increment_ref_count((struct buffer*)come_calloc(1, sizeof(struct buffer)*(1), "05parse.c", 376, "struct buffer*"))));
                if(!xisdigit(*info->p)) {
                ...
                line_2=0;
                fname_3=(struct buffer*)come_increment_ref_count(buffer_initialize((struct buffer*)come_increment_ref_count((struct buffer*)come_calloc(1, sizeof(struct buffer)*(1), "05parse.c", 426, "struct buffer*"))));
                while(xisdigit(*info->p)) {
