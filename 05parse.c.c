...
char* backtrace_parse_word(struct sInfo* info  )
{
    ...
    if(xisalpha(*info->p)||*info->p==95) {
        __dec_obj1=buf,
        buf=(char*)come_increment_ref_count((come_push_stackframe("05parse.c", 172, 2),__exception_result_var_b3=(come_push_stackframe("05parse.c", 172, 0),__exception_result_var_b1=parse_word(0,info), come_pop_stackframe(), __exception_result_var_b1), come_pop_stackframe(), __exception_result_var_b3));
    ...
    while(1) {
        if(*info->p==35) {
            ...
            if(parsecmp("pragma",info)) {
                buf=(struct buffer*)come_increment_ref_count(buffer_initialize((struct buffer*)come_increment_ref_count((struct buffer*)come_calloc_v2(1, sizeof(struct buffer)*(1), "05parse.c", 351, "struct buffer*"))));
...
_conditional_value_X0;})) {
                ...
                skip_spaces_and_tabs(info);
                line=0;
                fname=(struct buffer*)come_increment_ref_count(buffer_initialize((struct buffer*)come_increment_ref_count((struct buffer*)come_calloc_v2(1, sizeof(struct buffer)*(1), "05parse.c", 377, "struct buffer*"))));
                if(!xisdigit(*info->p)) {
                ...
                line_2=0;
                fname_3=(struct buffer*)come_increment_ref_count(buffer_initialize((struct buffer*)come_increment_ref_count((struct buffer*)come_calloc_v2(1, sizeof(struct buffer)*(1), "05parse.c", 427, "struct buffer*"))));
                while(xisdigit(*info->p)) {
