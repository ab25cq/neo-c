...
typedef struct anonymous_typeX5 PackedPair;
...
static struct anonymous_typeX5  make_pp(unsigned short int x, unsigned short int y, unsigned char a, unsigned char b);
int main();
...
int main()
{
    ...
    p=make_pp(1,2,3,4);
    do {
        if(!(sizeof(struct anonymous_typeX5 )==sizeof(unsigned int)+2)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","sizeof(PackedPair) == sizeof(unsigned int) + 2","code/c_test_compat25.nc",35);
            ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(__builtin_offsetof(struct anonymous_typeX5 , bytes)==sizeof(unsigned int))) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","offsetof(PackedPair, bytes) == sizeof(unsigned int)","code/c_test_compat25.nc",36);
            ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(p.x==1&&p.y==2)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","p.x == 1 && p.y == 2","code/c_test_compat25.nc",37);
            ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(p.bytes.a==3&&p.bytes.b==4)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","p.bytes.a == 3 && p.bytes.b == 4","code/c_test_compat25.nc",38);
            ok=(_Bool)0;
        }
    } while(0);
    m=({_A=(p.x); _B=(p.y); ((_A>_B)?(_A):(_B)); });
    do {
        if(!(m==2)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","m == 2","code/c_test_compat25.nc",41);
            ok=(_Bool)0;
        }
    } while(0);
    int arr[3]={1,2,3};
    s=({int (*pa)[3]=&arr; sum=0; for(i=0;i<sizeof(arr)/sizeof(arr[0]);i++){
sum+=(*pa)[i];
}
sum; });
    do {
        if(!(s==6)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","s == 6","code/c_test_compat25.nc",46);
            ok=(_Bool)0;
        }
    } while(0);
    puts(((ok)?("OK"):("NG")));
