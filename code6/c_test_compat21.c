...
unsigned char raw[sizeof(unsigned int)];
};
#pragma pack(pop)
#pragma pack(push, 1)
...
int main()
{
    ...
    do {
        if(!(pb.a==5)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","pb.a == 5","code/c_test_compat21.nc",26);
            ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(pb.b==7)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","pb.b == 7","code/c_test_compat21.nc",27);
            ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(pb.c==0xAA)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","pb.c == 0xAA","code/c_test_compat21.nc",28);
            ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(sizeof(struct PackedBits )>=2)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","sizeof(struct PackedBits) >= 2","code/c_test_compat21.nc",31);
            ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(__builtin_offsetof(struct PackedBits , c)>=1)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","offsetof(struct PackedBits, c) >= 1","code/c_test_compat21.nc",32);
            ok=(_Bool)0;
        }
    } while(0);
    puts(((ok)?("OK"):("NG")));
