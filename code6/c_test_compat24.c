...
int select(int __nfds, struct anonymous_typeX9* __restrict  __readfds  , struct anonymous_typeX9* __restrict  __writefds  , struct anonymous_typeX9* __restrict  __exceptfds  , struct timeval* __restrict  __timeout  );
int pselect(int __nfds, struct anonymous_typeX9* __restrict  __readfds  , struct anonymous_typeX9* __restrict  __writefds  , struct anonymous_typeX9* __restrict  __exceptfds  , const struct timespec* __restrict  __timeout  , const struct anonymous_typeX8* __restrict  __sigmask  );
long  int random();
...
struct anonymous_typeX5  div(int __numer, int __denom) __attribute__ ((__const__)) ;
struct anonymous_typeX6  ldiv(long  int __numer, long  int __denom) __attribute__ ((__const__)) ;
struct anonymous_typeX7  lldiv(long long int __numer, long long int __denom) __attribute__ ((__const__)) ;
...
int main()
{
    _Bool ok;
    unsigned long  n  ;
    union PackedFlex*  dyn  ;
    ok=(_Bool)1;
    union PackedFlex  pf  ={ .a = 5, .b = 7 };
    do {
        if(!(pf.a==5)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","pf.a == 5","code/c_test_compat24.nc",27);
            ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(pf.b==7)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","pf.b == 7","code/c_test_compat24.nc",28);
            ok=(_Bool)0;
        }
    } while(0);
    n=4;
    dyn=(union PackedFlex* )malloc(sizeof(unsigned short int)+n);
    do {
        if(!(dyn!=((void*)0))) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","dyn != NULL","code/c_test_compat24.nc",33);
            ok=(_Bool)0;
        }
    } while(0);
    if(dyn) {
        dyn->count=(unsigned short int)n;
        memcpy(dyn->data,"xyz",n);
        do {
            if(!(dyn->count==4)) {
                fprintf(stderr,"FAIL: %s at %s:%d\n","dyn->count == 4","code/c_test_compat24.nc",37);
                ok=(_Bool)0;
            }
        } while(0);
        do {
            if(!(strcmp(dyn->data,"xyz")==0)) {
                fprintf(stderr,"FAIL: %s at %s:%d\n","strcmp(dyn->data, \"xyz\") == 0","code/c_test_compat24.nc",38);
                ok=(_Bool)0;
            }
        } while(0);
        free(dyn);
