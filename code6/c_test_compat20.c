...
static struct PackedAnon  make_pa(int tag, unsigned char r, unsigned short int g, unsigned char b);
int main();
...
int main()
{
    ...
    ok=(_Bool)1;
    do {
        if(!(sizeof(struct PackedAnon )==sizeof(int)+sizeof(unsigned char)+sizeof(unsigned short int)+sizeof(unsigned char)+sizeof(int)+sizeof(int))) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","sizeof(struct PackedAnon) == sizeof(int) + sizeof(unsigned char) + sizeof(unsigned short) + sizeof(unsigned char) + sizeof(int) + sizeof(int)","code/c_test_compat20.nc",45);
            ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(__builtin_offsetof(struct PackedAnon , b)>__builtin_offsetof(struct PackedAnon , a))) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","offsetof(struct PackedAnon, b) > offsetof(struct PackedAnon, a)","code/c_test_compat20.nc",46);
            ok=(_Bool)0;
        }
    } while(0);
    pa=make_pa(1,1,0x0203,4);
    do {
        if(!(pa.tag==1)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","pa.tag == 1","code/c_test_compat20.nc",49);
            ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(pa.r==1&&pa.g==0x0203&&pa.blue==4)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","pa.r == 1 && pa.g == 0x0203 && pa.blue == 4","code/c_test_compat20.nc",50);
            ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(pa.a==10&&pa.b==20)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","pa.a == 10 && pa.b == 20","code/c_test_compat20.nc",51);
            ok=(_Bool)0;
        }
    } while(0);
    ...
    do {
        if(!(pa.tag==2)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","pa.tag == 2","code/c_test_compat20.nc",62);
            ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(pa.u==0x1111&&pa.v==0x22&&pa.w==0x33)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","pa.u == 0x1111 && pa.v == 0x22 && pa.w == 0x33","code/c_test_compat20.nc",63);
            ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(pa.a==-5&&pa.b==-6)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","pa.a == -5 && pa.b == -6","code/c_test_compat20.nc",64);
            ok=(_Bool)0;
        }
    } while(0);
    puts(((ok)?("OK"):("NG")));
