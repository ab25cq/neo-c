...
int aligned_global __attribute__((aligned(32)))=11;
int ignored_declspec=9;
// source head
...
__attribute__((noinline)) int noinline_prefix(int x);
...
int main()
{
    ...
    ok=(_Bool)1;
    do {
        if(!(_Alignof(struct DeclspecPrefix )>=16)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","_Alignof(struct DeclspecPrefix) >= 16","code/c_test_compat40.nc",31);
            ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(_Alignof(struct DeclspecMiddle )>=8)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","_Alignof(struct DeclspecMiddle) >= 8","code/c_test_compat40.nc",32);
            ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!((((unsigned long  int )&aligned_global)&31)==0)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","(((uintptr_t)&aligned_global) & 31) == 0","code/c_test_compat40.nc",33);
            ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(noinline_prefix(10)==11)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","noinline_prefix(10) == 11","code/c_test_compat40.nc",34);
            ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(noinline_suffix(10)==12)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","noinline_suffix(10) == 12","code/c_test_compat40.nc",35);
            ok=(_Bool)0;
        }
    } while(0);
    do {
        if(!(ignored_declspec==9)) {
            fprintf(stderr,"FAIL: %s at %s:%d\n","ignored_declspec == 9","code/c_test_compat40.nc",36);
            ok=(_Bool)0;
        }
    } while(0);
    puts(((ok)?("OK"):("NG")));
