        ...
        if(self.multiple_declare) {
            ...
            foreach(it, self.multiple_assign) {
                if(i < right_type.mGenericsTypes.length()) {
                    ...
                    add_variable_to_table(it, right_type2, info, false@function_param);
                }
            ...
            if(current_stack_frame_struct && info.lv_table.mVars.at(string(self.name), null) == null) {
    ...
    bool compile(sInfo* info)
    {
        ...
        if(come_value.type->mArrayNum.length() > 0) {
            if(info.in_typeof) {
                ...
                come_value.type->mOriginalLoadVarType = clone original_load_var_type;
            }
